...
- FFmpeg 8.x on Windows can treat finite MPEG-TS responses from Python `SimpleHTTPRequestHandler` as connection resets or hang while probing longer fixtures. Recording/DVR tests should use explicit `BaseHTTPRequestHandler` fixtures: send `Content-Length` for finite-file tests, and omit it only for deliberate live-style streams.

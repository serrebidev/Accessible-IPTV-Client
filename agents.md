...
- Some IPTV upstreams (signed-redirect CDNs like gohyperspeed → onebattlepitt) take well over 12 seconds to hand over their first HLS segment because of the TLS+redirect chain. `wait_for_playlist` must accept an `extended_timeout` and keep waiting as long as upstream bytes are *still arriving*; otherwise the proxy 503s mid-handshake on `chromecast_h264` (a fresh-session profile where bootstrap is intentionally suppressed) and Chromecast idles to ERROR. Base 25s with a 45s extended ceiling, plus a 3s no-progress stall guard, is the current tuning.
- `pychromecast.MediaController.block_until_active` returns when the receiver accepts the launch, not when playback begins. After it returns, poll `media_controller.status` for up to ~20 seconds and raise `PlaybackError` if the state stays IDLE or hits `idle_reason in ("ERROR", "INTERRUPTED")` — otherwise slow HLS warm-ups look like silent successes to the app while the user sees a dead receiver.

...
**Update 2026-02-01**: System tray Enter key now restores app with NVDA. The `EVT_TASKBAR_LEFT_DCLICK` event fires when pressing Enter on the tray icon, so double-click handling now calls `on_restore()`. Single-click remains disabled to prevent accidental focus stealing. Focus properly goes to channel list after restore via `wx.CallAfter()` with `Raise()` before `SetFocus()`.

...
controls = [frame.playlist_scope_combo, frame.group_list, frame.channel_list]
for reverse in (False, True):
    for index, control in enumerate(controls):
        ...
        app.Yield()
        assert wx.Window.FindFocus() == controls[(index + (-1 if reverse else 1)) % 3]
frame._exit_forced = True
frame.Close()
app.Yield()
print("Playlist choices, filtering and six Tab/Shift+Tab transitions: OK")
